...
| `truncate`        | No       | boolean | Truncate notification text. Disabled by default. Use `truncate_length` to set a custom length.<br>When a message is longer than 4000 characters and truncation is disabled, it is automatically truncated to 4000 characters to guarantee delivery.  |
